...
file_path = r"D:\dev_10 rho_ERT_25_Wendelsheim\Constant filter\20250625_dev_rho.xlsx"
...
# Create empty 'k' column
df['k'] = np.nan

# -----------------------------
# Define the DD row index ranges (convert from Excel row numbers)
# Excel rows: 2–6477  → Python: 1–6476
# Excel rows: 10767–15361 → Python: 10766–15360
# -----------------------------
dd_ranges = [(1, 6476), (10766, 15360)]

# -----------------------------
# Define the MG row index ranges (convert from Excel row numbers)
# Excel rows: 6478–10766 → Python: 6477–10765
# Excel rows: 15362–18482 → Python: 15361–18481
# -----------------------------
mg_ranges = [(6477, 10765), (15361, 18481)]

# -----------------------------
# Function to compute K given the formula
# -----------------------------
def compute_K(B, C, D, E, formula_type):
    if formula_type == 'DD':
        term1 = 1 / abs(B - D)
        term2 = 1 / abs(C - D)
        term3 = 1 / abs(B - E)
        term4 = 1 / abs(C - E)
        return 2 * np.pi / (term1 - term2 - term3 + term4)
    elif formula_type == 'MG':
        term1 = 1 / abs(B - C)
        term2 = 1 / abs(C - E)
        term3 = 1 / abs(B - D)
        term4 = 1 / abs(D - E)
...
# -----------------------------
# Compute K for DD ranges
# -----------------------------
for start, end in dd_ranges:
    B, C, D, E = df.loc[start:end, 'Spa1'], df.loc[start:end, 'Spa2'], df.loc[start:end, 'Spa3'], df.loc[start:end, 'Spa4']
    df.loc[start:end, 'k'] = compute_K(B, C, D, E, 'DD')

# -----------------------------
# Compute K for MG ranges
# -----------------------------
for start, end in mg_ranges:
    B, C, D, E = df.loc[start:end, 'Spa1'], df.loc[start:end, 'Spa2'], df.loc[start:end, 'Spa3'], df.loc[start:end, 'Spa4']
    df.loc[start:end, 'k'] = compute_K(B, C, D, E, 'MG')
...
# # Saving the file
output_path = r"D:\dev_10 rho_ERT_25_Wendelsheim\Constant filter\_20250625_wendelsheim_filtered.xlsx"
...
file_path =  r"D:\dev_10 rho_ERT_25_Wendelsheim\Constant filter\_20250625_wendelsheim_filtered.xlsx"
df = pd.read_excel(file_path) # Remember the name :df
...
output_dir = r"D:\dev_10 rho_ERT_25_Wendelsheim\8 filters"
...
for name, condition in filters.items():
    ...
    output_file = os.path.join(output_dir, f"_20250625_{name}.xlsx")
